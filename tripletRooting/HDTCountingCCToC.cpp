...
void HDT::handleCCToC()
{
    ...
    for (int i = 0; i < this->degree; i++){
        ...
        tripResolved_root += c1Count->get_n_i(0) * c2Count->get_n_i_arrow_circ(0) + (c1->circ - c1Count->get_n_i(0)) * c2Count->get_n_arrow_ij(i,0);
    }

    tripResolved_root += Util::binom2(c1Count->get_n_i(0)) * (c2->circ - c2Count->get_n_i(0)) + Util::binom2(c2->circ - c2->get_n_i(0)) * c1->get_n_i(0);
    tripResolved_root += (c1->circ - c1Count->get_n_i(0)) * c2Count->get_n_parent_ii(0) + c1Count->get_n_i(0) * (c2->n_parent_circ_square - c2->n_parent_zero_circ);
    ...
    for (int j = 0; j < this->degree; j++){
        for (int i = 0; i < this->degree; i++){
            if (i != j){ // leave j out
                ...
                tripResolved[j] += Util::binom2(c1Count->get_n_i(i)) * ( c2->circ - c2Count->get_n_i(j) - c2Count->get_n_i(i) );
                tripResolved[j] += ( c1->circ - c1Count->get_n_i(j) - c1Count->get_n_i(i) ) * c2Count->get_n_parent_ii(i);
                tripUnresolved[j] += c1->get_n_i(i) * ( c2->circ_square - c2Count->get_n_i_circ(j) - c2Count->get_n_i_circ(i) + c2Count->get_n_ij(i,j) );
            }
        }
    }

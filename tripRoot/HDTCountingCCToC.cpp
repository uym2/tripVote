...
void HDT::handleCCToC()
{
    ...
    tripResolved_root = c1->tripResolved_root + c2->tripResolved_root;

    for (int i = 0; i < this->degree; i++){
        tripResolved[i] = c1->tripResolved[i] + c2->tripResolved[i];
	    tripUnresolved[i] = c1->tripUnresolved[i] + c2->tripUnresolved[i];
        tripResolved_root += c1Count->get_n_i(0) * c2Count->get_n_i_arrow_circ(0) + (c1->n_circ - c1Count->get_n_i(0)) * c2Count->get_n_arrow_ij(i,0);
    }

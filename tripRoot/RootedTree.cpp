	...
	for(vector<RootedTree*>::iterator i = l->begin(); i != l->end(); i++)
	{
		...
		if (j == altWorldEnd)
		{
            ...
            if (do_pruning){
                ...
                cerr << leaf->name << " didn't exist in second tree. Pruning it out ..." << endl;
                this->prune_subtree(leaf);
                continue;
            } else {

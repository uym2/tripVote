...
TripletDistanceCalculator : private AbstractDistanceCalculator {
 public:
  ...
  std::vector<std::vector<double> > calculateAllPairsTripletDistance(const char *filename, bool normalized=false);
  std::vector<std::vector<double> > calculateAllPairsTripletDistance(std::vector<UnrootedTree *> trees, bool normalized=false);
